...
def build_poster_modern(anime, photo_bytes, brand, theme_name="purple"):
    ...
    # Reduced blur — keep background recognizable (was 30, now 18)
    bg_blur = bg.filter(ImageFilter.GaussianBlur(18))

    # Lighter dark overlay — 52% so bg shows through more (was 0.62)
    dark = Image.new("RGB", (W, H), (8, 6, 16))
    base = Image.blend(bg_blur, dark, 0.52)
    ...
    # ── Brand badge — glossy gradient with glow ──
    badge_text = f"✦ {brand.upper()}"
    f_badge = load_font(13, True)
    btw = draw.textbbox((0,0), badge_text, font=f_badge)[2]
    bw, bh = btw + 40, 36   # slightly wider, taller
    bx, by = LEFT, BADGE_Y

    # Base filled pill with theme color
    ...
    # Left-to-right gradient: lighter on left, darker on right
    for gx_b in range(bw):
        t = gx_b / bw
        r = int(min(255, ACCENT[0] + (1-t)*60))
        g_ = int(min(255, ACCENT[1] + (1-t)*60))
        b_ = int(min(255, ACCENT[2] + (1-t)*60))
        ImageDraw.Draw(badge_img).line([(gx_b,0),(gx_b,bh)], fill=(r,g_,b_))

    # Subtle top highlight (glossy inner shine)
    highlight = Image.new("RGBA", (bw, bh//2), (255,255,255,35))
    hmask = Image.new("L", (bw, bh//2), 0)
    ImageDraw.Draw(hmask).rounded_rectangle([0,0,bw,bh//2], radius=bh//2, fill=255)
    badge_img.paste(Image.new("RGB",(bw,bh//2),(255,255,255)),
                    (0,0), Image.composite(hmask, Image.new("L",(bw,bh//2),0), hmask))

    # Outer glow behind badge
    glow_sz = 12
    glow = Image.new("RGBA", (bw+glow_sz*2, bh+glow_sz*2), (0,0,0,0))
    ImageDraw.Draw(glow).rounded_rectangle(
        [glow_sz,glow_sz,bw+glow_sz,bh+glow_sz],
        radius=bh//2+glow_sz//2,
        fill=(ACCENT[0],ACCENT[1],ACCENT[2],80)
    )
    glow = glow.filter(ImageFilter.GaussianBlur(8))
    base.paste(Image.new("RGB",glow.size,(0,0,0)),
               (bx-glow_sz, by-glow_sz), glow.split()[3])
    ...
    # dark tint — slightly darker for better contrast (was 155, now 175)
