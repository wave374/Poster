...
ASK_ANIME, ASK_CONFIRM, ASK_PHOTO, ASK_BRAND, ASK_STYLE, ASK_COLOR = range(6)
...
def build_poster_ui(anime: dict, photo_bytes: bytes, brand: str, theme_name: str = "cyan") -> bytes:
    ...
    ImageDraw.Draw(sb_mask).rounded_rectangle([0, 0, SB_W - 1, H - 1], radius=RADIUS, fill=255)
    ...
    for i, (icon, label) in enumerate(nav):
        ...
        draw.text((SB_W // 2, iy),      icon,  font=load_font(24, bold=True), fill=col, anchor="mm")
        draw.text((SB_W // 2, iy + 28), label, font=load_font(10, bold=True), fill=col, anchor="mm")
    ...
    TC_H = 148
    _glass_card(base, CX, TC_Y, LEFT_W, TC_H, ACCENT, border=False, alpha=165, radius=RADIUS)
    draw = ImageDraw.Draw(base)
    ...
    if season_text:
        draw.text((CX + PAD, ty_center + title_h + 6),
                  season_text, font=load_font(int(f_title.size * 0.36), bold=True), fill=ACCENT)

    # ── 6. INFORMATION CARD ──
    IC_Y = TC_Y + TC_H + GAP
    IC_H = H - IC_Y - OUTER
    _glass_card(base, CX, IC_Y, LEFT_W, IC_H, ACCENT, border=False, alpha=165, radius=RADIUS)
    draw = ImageDraw.Draw(base)

    draw.text((CX + PAD, IC_Y + PAD), "INFORMATION", font=load_font(18, bold=True), fill=WHITE)
    info_items  = [f"STUDIO : {studio}", f"STATUS : TV ({episodes} EPS)", f"RATINGS : {score}"]
    bullet_area = IC_H - PAD * 2 - 32
    bullet_gap  = bullet_area // (len(info_items) + 1)
    for ii, item in enumerate(info_items):
        by = IC_Y + PAD + 32 + (ii + 1) * bullet_gap - 8
        draw.text((CX + PAD, by), f"• {item}", font=load_font(16), fill=WHITE)
    ...
    ImageDraw.Draw(rmask).rounded_rectangle([0, 0, RIGHT_W - 1, FI_H - 1], radius=RADIUS, fill=255)
    ...
    syn_w = RIGHT_W // 15 + 2
    lines = textwrap.wrap(syn_clean, width=syn_w)
    syn_y = FL_Y + 32
    for line in lines[:4]:
        draw.text((RX, syn_y), line, font=load_font(15), fill=GRAY)
        syn_y += 26
    if len(lines) > 4:
        draw.text((RX, syn_y), "...read more", font=load_font(15), fill=ACCENT)
        syn_y += 26
    ...
    OV_H = H - OV_Y - OUTER
    if OV_H >= 80:
        _glass_card(base, RX, OV_Y, RIGHT_W, OV_H, ACCENT, border=False, alpha=165, radius=RADIUS)
        draw = ImageDraw.Draw(base)
        draw.text((RX + PAD, OV_Y + PAD), "OVERVIEW", font=load_font(17, bold=True), fill=WHITE)
        ov_lines = textwrap.wrap(syn_clean[120:], width=syn_w)[:4]
        for ii, line in enumerate(ov_lines):
            draw.text((RX + PAD, OV_Y + PAD + 32 + ii * 26), line, font=load_font(14), fill=GRAY)
...
async def received_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Ask poster style first
    buttons = [
        [InlineKeyboardButton("🎬 Sᴛʏʟᴇ 1 — Cɪɴᴇᴍᴀᴛɪᴄ", callback_data="style_cinematic")],
        [InlineKeyboardButton("🖥️ Sᴛʏʟᴇ 2 — Dᴀsʜʙᴏᴀʀᴅ UI", callback_data="style_ui")],
    ]
    await update.message.reply_text(
        "🖼️ Choose your *poster style*:\n\n"
        "🎬 *Style 1* — Cinematic: big title, portrait image, glassmorphism card\n"
        "🖥️ *Style 2* — Dashboard UI: sidebar, info cards, featured image",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
    return ASK_STYLE
...
async def color_selected(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    style      = ctx.user_data.get("style", "cinematic")
    style_label = "Dashboard UI" if style == "ui" else "Cinematic"
    await query.edit_message_text(
        f"🎨 Creating your *{theme_name}* {style_label} poster, please wait...",
        parse_mode="Markdown"
    )
    try:
        ...
        await query.message.reply_photo(
            ...
            caption=(
                f"🎌 *{title}*\n"
                f"{THEMES[theme_name]['emoji']} {theme_name.capitalize()} · "
                f"{'🖥️ Dashboard UI' if style == 'ui' else '🎬 Cinematic'} poster ready\\!\n\n"
                f"Send /anime to make another\\."
            ),
