...
# ─── CONFIG ───────────────────────────────────────────────────────────────────
BOT_TOKEN  = "8620146968:AAEu6CC8rJUjnRXD2iO7SxDVEVEu3hZWh78"
...
def build_poster_ui(anime: dict, photo_bytes: bytes, brand: str, theme_name: str = "cyan") -> bytes:
    ...
    # ── Base: blurred photo background, lightly darkened ──
    bg = Image.open(io.BytesIO(photo_bytes)).convert("RGB")
    bg = bg.resize((W, H), Image.LANCZOS)
    bg = bg.filter(_IF.GaussianBlur(12))
    # subtle dark overlay so cards stand out
    dark = Image.new("RGB", (W, H), (10, 10, 18))
    base = Image.blend(bg, dark, 0.55)

    draw = ImageDraw.Draw(base)

    # ── Sidebar: dark strip, no border ──
    sb_w = 88
    sidebar_img = Image.new("RGBA", (sb_w, H), (12, 12, 22, 210))
    base.paste(sidebar_img.convert("RGB"), (0, 0),
               sidebar_img.split()[3])
    draw = ImageDraw.Draw(base)

    # Hamburger + brand
    draw.text((sb_w // 2, 22), "≡", font=load_font(26, bold=True), fill=WHITE, anchor="mm")
    draw.text((sb_w // 2, 46), brand[:4].upper(), font=load_font(10, bold=True), fill=WHITE, anchor="mm")

    # Nav icons
    nav = [("⌂", "HOME"), ("⌕", "SEARCH"), ("☰", "CATEGORY"), ("★", "RATING")]
    for i, (icon, label) in enumerate(nav):
        iy = 110 + i * 110
        col = ACCENT if i == 0 else (130, 130, 150)
        draw.text((sb_w // 2, iy), icon, font=load_font(22, bold=True), fill=col, anchor="mm")
        draw.text((sb_w // 2, iy + 24), label, font=load_font(9, bold=True), fill=col, anchor="mm")
    ...
    for g in genres:
        ...
        pill = Image.new("RGBA", (pw, ph), (255, 255, 255, 40))
        pmask = Image.new("L", (pw, ph), 0)
        ImageDraw.Draw(pmask).rounded_rectangle([0, 0, pw, ph], radius=ph // 2, fill=255)
        base.paste(pill.convert("RGB"), (gx, gy), pmask)
        draw = ImageDraw.Draw(base)
        draw.rounded_rectangle([gx, gy, gx + pw, gy + ph], radius=ph // 2, outline=WHITE, width=2)
        draw.text((gx + pw // 2, gy + ph // 2), g, font=f_genre, fill=WHITE, anchor="mm")
        gx += pw + 12
    ...
    # Title card (glass, no border)
    tc_y, tc_h = 68, 175
    _glass_card(base, lx, tc_y, lw, tc_h, ACCENT, border=False)
    draw = ImageDraw.Draw(base)

    f_title = load_font(88, bold=True)
    while f_title.size > 36:
        bbox = draw.textbbox((0, 0), clean_title.upper(), font=f_title)
        if (bbox[2] - bbox[0]) < lw - 24:
            break
        f_title = load_font(f_title.size - 5, bold=True)
    draw.text((lx + 18, tc_y + tc_h // 2), clean_title.upper(),
              font=f_title, fill=WHITE, anchor="lm")
    if season_text:
        draw.text((lx + 18, tc_y + tc_h // 2 + f_title.size // 2 + 4),
                  season_text, font=load_font(int(f_title.size * 0.38), bold=True),
                  fill=ACCENT)

    # Info card (glass, no border, fills rest of left col)
    ic_y = tc_y + tc_h + 14
    ic_h = H - ic_y - 18
    _glass_card(base, lx, ic_y, lw, ic_h, ACCENT, border=False)
    draw = ImageDraw.Draw(base)

    f_ih = load_font(17, bold=True)
    f_i  = load_font(15)
    draw.text((lx + 18, ic_y + 18), "INFORMATION", font=f_ih, fill=WHITE)
    for ii, item in enumerate([
        f"STUDIO : Unknown",
        f"STATUS : TV ({episodes} EPS)",
        f"RATINGS : {score}",
    ]):
        draw.text((lx + 18, ic_y + 54 + ii * 38), f"• {item}", font=f_i, fill=WHITE)

    # ── Right column ──
    rx = lx + lw + 16
    rw = W - rx - 14

    # Featured image — fills top right, rounded corners, NO border
    ri_h = int(H * 0.50)
    feat = Image.open(io.BytesIO(photo_bytes)).convert("RGB")
    feat = feat.resize((rw, ri_h), Image.LANCZOS)
    rmask = Image.new("L", (rw, ri_h), 0)
    ImageDraw.Draw(rmask).rounded_rectangle([0, 0, rw, ri_h], radius=14, fill=255)
    base.paste(feat, (rx, 68), rmask)
    draw = ImageDraw.Draw(base)

    # FEATURED label + synopsis snippet
    fl_y = 68 + ri_h + 10
    draw.text((rx, fl_y), "FEATURED", font=load_font(16, bold=True), fill=WHITE)
    for ii, line in enumerate(textwrap.wrap(syn_clean, 42)[:3]):
        draw.text((rx, fl_y + 26 + ii * 22), line, font=load_font(13), fill=GRAY)

    # Overview card (glass, no border)
    ov_y = fl_y + 26 + 3 * 22 + 10
    ov_h = H - ov_y - 14
    if ov_h > 55:
        _glass_card(base, rx, ov_y, rw, ov_h, ACCENT, border=False)
        draw = ImageDraw.Draw(base)
        draw.text((rx + 14, ov_y + 12), "OVERVIEW", font=load_font(15, bold=True), fill=WHITE)
        for ii, line in enumerate(textwrap.wrap(syn_clean[100:], 36)[:3]):
            draw.text((rx + 14, ov_y + 36 + ii * 22), line, font=load_font(12), fill=GRAY)
...
def _glass_card(img, x, y, w, h, accent, border=True):
    """Frosted dark glass card — no hard border by default."""
    card = Image.new("RGBA", (w, h), (15, 15, 28, 185))
    ...
    if border:
        ImageDraw.Draw(img).rounded_rectangle(
            [x, y, x + w, y + h], radius=14, outline=accent, width=2)
