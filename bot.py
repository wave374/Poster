...
def build_poster_modern(anime, photo_bytes, brand, theme_name="purple"):
    ...
    bg_blur = bg.filter(ImageFilter.GaussianBlur(18))  # reduced blur
    base = Image.blend(bg_blur, Image.new("RGB",(W,H),(8,6,16)), 0.52)  # lighter overlay
...
def build_poster_banner(anime, photo_bytes, brand, theme_name="green"):
    ...
    # Character image with left-to-right fade
    char = Image.open(io.BytesIO(photo_bytes)).convert("RGB")
    cr = char.width / char.height
    if cr > W / H:
        nw = int(char.height * (W / H))
        char = char.crop(((char.width-nw)//2, 0, (char.width-nw)//2+nw, char.height))
    else:
        nh = int(char.width / (W / H))
        char = char.crop((0, (char.height-nh)//2, char.width, (char.height-nh)//2+nh))
    char = char.resize((W, H), Image.LANCZOS)

    # Fast PIL-based horizontal fade (no pixel loop)
    fade_start = int(W * 0.30)
    fade_end   = int(W * 0.55)
    ...
    char_rgba = char.convert("RGBA")
    r, g, b, _ = char_rgba.split()
    char_rgba = Image.merge("RGBA", (r, g, b, fade_mask))
    base_rgba = base.convert("RGBA")
    base_rgba.paste(char_rgba, (0, 0), char_rgba)
    base = base_rgba.convert("RGB")
    ...
    for i in range(60):
        ...
        vd.rectangle([0, H-i-1, W, H-i], fill=(0,0,0,a))
    base = Image.alpha_composite(base.convert("RGBA"), vig).convert("RGB")
    draw = ImageDraw.Draw(base)

    # TOP CENTER: spaced text spanning ~70% of width
    ...
    # "!! SYNOPSIS" header
    f_syn_hdr = load_font(20, True)
    sy_hdr_y  = title_bottom + 14
    draw.text((LEFT,      sy_hdr_y), "‼",        font=f_syn_hdr, fill=(220,50,50))
    draw.text((LEFT + 36, sy_hdr_y), " SYNOPSIS", font=f_syn_hdr, fill=ACCENT)

    # Synopsis text
    f_syn   = load_font(15)
    wrapped = textwrap.wrap(syn_clean, width=58)
    sy = sy_hdr_y + 36
    for line in wrapped[:3]:
        draw.text((LEFT, sy), line, font=f_syn, fill=(210,210,210))
        sy += 26
    if len(wrapped) > 3:
        draw.text((LEFT, sy), "...read more", font=f_syn, fill=(170,170,170))
        sy += 26

    # Genre tags — plain bold text, spaced
    gy = sy + 10
    gx = LEFT
    f_genre = load_font(16, True)
    for g in genres:
    ...
    # Brand tag — transparent glowing pill with diamond bullet + spaced text
    badge_y   = line_y + 18
    f_pill    = load_font(14, True)
    # Build label with diamond bullet and letter-spaced brand
    spaced_brand = "  ".join(list(brand.upper()))
    pill_text = f"◆   {spaced_brand}"
    ...
    # Pill body — semi-transparent white fill
    ...
    # Translucent white fill (like frosted glass)
    ...
    # Text with soft glow — draw glow first then crisp text on top
    text_x = pill_x + 24
    text_y = pill_y + pill_h2//2
    glow_text_layer = Image.new("RGBA", (W, H), (0,0,0,0))
    ImageDraw.Draw(glow_text_layer).text((text_x, text_y), pill_text, font=f_pill,
                                         fill=(255,255,255,120), anchor="lm")
    glow_text_layer = glow_text_layer.filter(ImageFilter.GaussianBlur(4))
    base.alpha_composite(glow_text_layer)
