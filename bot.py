...
# ─── HELPERS ──────────────────────────────────────────────────────────────────
def _glass_card(img, x, y, w, h, accent, border=False, alpha=175, radius=20):
    """Frosted glass card with strong backdrop blur and consistent 20px radius."""
    # Blur the region behind the card for glassmorphism
    region = img.crop((x, y, x + w, y + h)).convert("RGB")
    blurred = region.filter(ImageFilter.GaussianBlur(6))
    # Dark glass overlay
    card = Image.new("RGBA", (w, h), (18, 18, 32, alpha))
    mask = Image.new("L", (w, h), 0)
    ImageDraw.Draw(mask).rounded_rectangle([0, 0, w - 1, h - 1], radius=radius, fill=255)
    blurred_rgba = blurred.convert("RGBA")
    result = Image.alpha_composite(blurred_rgba, card)
    img.paste(result.convert("RGB"), (x, y), mask)
    if border:
        ImageDraw.Draw(img).rounded_rectangle(
            [x, y, x + w - 1, y + h - 1], radius=radius, outline=accent, width=2)

def _draw_card(img, x, y, w, h, accent):
    _glass_card(img, x, y, w, h, accent, border=True)

def _pill_button(base, draw, x, y, label, font, height=52, pad_x=28, radius=30):
    """Draw a white outlined pill button — consistent height, radius, padding."""
    tw = draw.textbbox((0, 0), label, font=font)[2]
    pw = tw + pad_x * 2
    pill = Image.new("RGBA", (pw, height), (255, 255, 255, 35))
    pmask = Image.new("L", (pw, height), 0)
    ImageDraw.Draw(pmask).rounded_rectangle([0, 0, pw - 1, height - 1], radius=radius, fill=255)
    base.paste(pill.convert("RGB"), (x, y), pmask)
    draw = ImageDraw.Draw(base)
    draw.rounded_rectangle([x, y, x + pw - 1, y + height - 1],
                            radius=radius, outline=WHITE, width=2)
    draw.text((x + pw // 2, y + height // 2), label, font=font, fill=WHITE, anchor="mm")
    return pw  # return width for advancing x

# ─── POSTER STYLE 2: DASHBOARD UI (FIXED) ─────────────────────────────────────
def build_poster_ui(anime: dict, photo_bytes: bytes, brand: str, theme_name: str = "cyan") -> bytes:
    ...
    # ── 8pt spacing system ──
    OUTER  = 32   # outer margin from canvas edge
    GAP    = 24   # gutter between cards / columns
    PAD    = 28   # internal card padding
    RADIUS = 20   # border radius everywhere

    # ── 1. BACKGROUND: blurred photo + dark overlay ──
    ...
    base = Image.blend(bg, dark, 0.58)

    # ── 2. SIDEBAR (wider, better proportioned) ──
    SB_W = 106   # ~20% wider than before
    sidebar = Image.new("RGBA", (SB_W, H), (12, 13, 26, 220))
    sb_mask = Image.new("L", (SB_W, H), 0)
    # Right side rounded
    ImageDraw.Draw(sb_mask).rounded_rectangle(
        [0, 0, SB_W - 1, H - 1], radius=RADIUS, fill=255)
    region = base.crop((0, 0, SB_W, H)).convert("RGBA")
    sb_blended = Image.alpha_composite(region, sidebar)
    base.paste(sb_blended.convert("RGB"), (0, 0), sb_mask)

    draw = ImageDraw.Draw(base)

    # Hamburger icon + brand name
    draw.text((SB_W // 2, 28), "≡", font=load_font(28, bold=True), fill=WHITE, anchor="mm")
    draw.text((SB_W // 2 + 4, 28), f" {brand}", font=load_font(13, bold=True),
              fill=WHITE, anchor="lm")

    # Nav icons — equal spacing, centered icons, proper labels
    nav = [("⌂", "HOME"), ("⌕", "SEARCH"), ("☰", "CATEGORY"), ("★", "RATING")]
    nav_start = 130
    nav_gap   = (H - nav_start - 80) // len(nav)
    for i, (icon, label) in enumerate(nav):
        iy   = nav_start + i * nav_gap
        col  = ACCENT if i == 0 else (110, 115, 140)
        icon_font  = load_font(24, bold=True)
        label_font = load_font(10, bold=True)
        draw.text((SB_W // 2, iy),       icon,  font=icon_font,  fill=col, anchor="mm")
        draw.text((SB_W // 2, iy + 28),  label, font=label_font, fill=col, anchor="mm")

    # ── 3. CONTENT AREA bounds ──
    CX    = SB_W + OUTER          # content left edge
    CW    = W - CX - OUTER        # total content width
    # 58 / 42 left / right split
    LEFT_W  = int(CW * 0.575)
    RIGHT_W = CW - LEFT_W - GAP
    RX      = CX + LEFT_W + GAP   # right column left edge
    TOP_Y   = OUTER               # top of content
    ...
    gx       = CX
    for g in genres:
        pw = _pill_button(base, ImageDraw.Draw(base), gx, TOP_Y, g, f_genre,
                          height=PILL_H, pad_x=28, radius=PILL_H // 2)
        gx += pw + GAP
    GENRE_BOTTOM = TOP_Y + PILL_H

    # ── 5. TITLE CARD ──
    TC_Y = GENRE_BOTTOM + GAP
    TC_H = 148    # fixed balanced height
    _glass_card(base, CX, TC_Y, LEFT_W, TC_H, ACCENT, border=False,
                alpha=165, radius=RADIUS)
    draw = ImageDraw.Draw(base)

    f_title = load_font(80, bold=True)
    while f_title.size > 32:
        bbox = draw.textbbox((0, 0), clean_title.upper(), font=f_title)
        if (bbox[2] - bbox[0]) <= LEFT_W - PAD * 2:
            break
        f_title = load_font(f_title.size - 4, bold=True)

    # Vertically center title text in card
    title_h = draw.textbbox((0, 0), clean_title.upper(), font=f_title)[3]
    ty_center = TC_Y + (TC_H - title_h) // 2
    draw.text((CX + PAD, ty_center), clean_title.upper(), font=f_title, fill=WHITE)
    if season_text:
        draw.text((CX + PAD, ty_center + title_h + 6),
                  season_text, font=load_font(int(f_title.size * 0.36), bold=True),
                  fill=ACCENT)

    # ── 6. INFORMATION CARD ──
    IC_Y = TC_Y + TC_H + GAP
    IC_H = H - IC_Y - OUTER      # fills to bottom with outer margin
    _glass_card(base, CX, IC_Y, LEFT_W, IC_H, ACCENT, border=False,
                alpha=165, radius=RADIUS)
    draw = ImageDraw.Draw(base)

    f_head = load_font(18, bold=True)
    f_info = load_font(16)
    draw.text((CX + PAD, IC_Y + PAD), "INFORMATION", font=f_head, fill=WHITE)
    ...
    # Space bullet items evenly inside card
    bullet_area = IC_H - PAD * 2 - 32   # subtract header
    bullet_gap  = bullet_area // (len(info_items) + 1)
    for ii, item in enumerate(info_items):
        by = IC_Y + PAD + 32 + (ii + 1) * bullet_gap - 8
        draw.text((CX + PAD, by), f"• {item}", font=f_info, fill=WHITE)

    # ── 7. RIGHT COLUMN — FEATURED IMAGE ──
    # Image height = ~38% of total H (balanced, not oversized)
    FI_H = int(H * 0.385)
    feat = Image.open(io.BytesIO(photo_bytes)).convert("RGB")
    # Smart crop: fill the right column width
    feat_scale = RIGHT_W / feat.width
    feat_h_scaled = int(feat.height * feat_scale)
    feat = feat.resize((RIGHT_W, max(feat_h_scaled, FI_H)), Image.LANCZOS)
    if feat.height > FI_H:
        # Crop top-focused to keep face
        feat = feat.crop((0, 0, RIGHT_W, FI_H))
    rmask = Image.new("L", (RIGHT_W, FI_H), 0)
    ImageDraw.Draw(rmask).rounded_rectangle(
        [0, 0, RIGHT_W - 1, FI_H - 1], radius=RADIUS, fill=255)
    base.paste(feat, (RX, TOP_Y), rmask)
    draw = ImageDraw.Draw(base)

    # ── 8. FEATURED label + synopsis ──
    FL_Y = TOP_Y + FI_H + GAP
    draw.text((RX, FL_Y), "FEATURED", font=load_font(18, bold=True), fill=WHITE)

    f_syn   = load_font(15)
    syn_y   = FL_Y + 32
    syn_w   = RIGHT_W // 15 + 2   # chars per line
    lines   = textwrap.wrap(syn_clean, width=syn_w)[:4]
    for line in lines:
        draw.text((RX, syn_y), line, font=f_syn, fill=GRAY)
        syn_y += 26
    if len(textwrap.wrap(syn_clean, width=syn_w)) > 4:
        draw.text((RX, syn_y), "...read more", font=f_syn, fill=ACCENT)
        syn_y += 26
    ...
    OV_H = H - OV_Y - OUTER
    if OV_H >= 80:
        _glass_card(base, RX, OV_Y, RIGHT_W, OV_H, ACCENT, border=False,
                    alpha=165, radius=RADIUS)
        draw = ImageDraw.Draw(base)
        draw.text((RX + PAD, OV_Y + PAD), "OVERVIEW",
                  font=load_font(17, bold=True), fill=WHITE)
        ov_lines = textwrap.wrap(syn_clean[120:], width=syn_w)[:4]
        for ii, line in enumerate(ov_lines):
            draw.text((RX + PAD, OV_Y + PAD + 32 + ii * 26),
                      line, font=load_font(14), fill=GRAY)
