...
def build_poster(anime: dict, photo_bytes: bytes, brand: str) -> bytes:
    ...
    draw.text((52, 20), brand.upper(), font=load_font(16, bold=True), fill=WHITE)
    ...
    title_bottom = draw.textbbox((38, 180), title_upper, font=f_title)[3]

    # subtitle
    if title_jp and title_jp != title:
        f_sub = load_font(int(f_title.size * 0.88), bold=True)
        draw.text((38, title_bottom + 4), title_jp.upper(), font=f_sub, fill=GREEN)
        title_bottom = draw.textbbox((38, title_bottom + 4), title_jp.upper(), font=f_sub)[3]

    # genres
    gy, gx = title_bottom + 18, 38
    ...
    # score & episodes
    badge_x, badge_y = 38, gy + 32
    for label, val in [("★", str(score)), ("EP", str(episodes))]:
        txt = f"{label} {val}"
        draw.text((badge_x, badge_y), txt, font=load_font(13, bold=True), fill=GREEN)
    ...
    for i, label in enumerate(["DOWNLOAD", "JOIN NOW"]):
        bx = 38 + i * 210
        bw, bh = 185, 44
        draw.rounded_rectangle([bx, btn_y, bx + bw, btn_y + bh], radius=4, fill=GREEN)
        f_btn = load_font(15, bold=True)
        tw = draw.textbbox((0, 0), label, font=f_btn)[2]
        draw.text((bx + (bw - tw) // 2, btn_y + 12), label, font=f_btn, fill=BLACK)
