...
def build_poster_modern(anime, photo_bytes, brand, theme_name="purple"):
    ...
    bg = Image.open(io.BytesIO(photo_bytes)).convert("RGB").resize((W,H), Image.LANCZOS)
    bg_blur = bg.filter(ImageFilter.GaussianBlur(28))
    base = Image.blend(bg_blur, Image.new("RGB",(W,H),(10,8,18)), 0.72)
    ...
    ImageDraw.Draw(badge_bg).rounded_rectangle([0,0,bw,bh], radius=bh//2, fill=ACCENT)
    ...
    # Synopsis card with left bar
    card_x, card_y = LEFT, title_bottom+20
    card_w, card_h = int(W*0.50), 110
    bar_w = 4
    region = base.crop((card_x,card_y,card_x+card_w,card_y+card_h))
    ...
    base.paste(tint,(card_x,card_y),
               Image.composite(Image.new("L",(card_w,card_h),140),Image.new("L",(card_w,card_h),0),cmask))
    draw = ImageDraw.Draw(base)
    ...
    ImageDraw.Draw(shadow).rounded_rectangle([sh_sz,sh_sz,pw_img+sh_sz,ph_img+sh_sz], radius=20, fill=(0,0,0,120))
    shadow = shadow.filter(ImageFilter.GaussianBlur(12))
    base.paste(Image.new("RGB",shadow.size,(0,0,0)),(px-sh_sz,py-sh_sz),shadow.split()[3])
    base.paste(p_img,(px,py),rmask)
    draw = ImageDraw.Draw(base)
    draw.rounded_rectangle([px,py,px+pw_img,py+ph_img], radius=18, outline=(80,70,100), width=1)
...
# ── THE FIX: cmd_anime_callback sends message then bot waits for text reply ──
async def cmd_anime_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ctx.user_data.clear()
    ctx.user_data["waiting_anime"] = True
...
async def received_anime_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # handle both /anime command flow and button flow
    if ctx.user_data.get("waiting_anime"):
        ctx.user_data.pop("waiting_anime")
        # re-enter conversation manually
