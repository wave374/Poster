...
def build_poster(anime: dict, photo_bytes: bytes, brand: str, color: str = "color_green") -> bytes:
    ...
    # Draw hex grid with theme colors
    hex_size = 55
    hex_w = hex_size * 2
    hex_h = math.sqrt(3) * hex_size
    col_w = hex_w * 0.75
    ...
    for col in range(cols):
        for row in range(rows):
            ...
            grid_draw.polygon(points, outline=accent, fill=hfill)
    ...
    for x in range(int(W * 0.52)):
        ...
        ov_draw.line([(x, 0), (x, H)], fill=(*ovlay, alpha))
   ...
   # ── Step 6: Season number in accent color ──
    import re as _re
    ...
    if season_match:
        ...
        draw.text((38, title_bottom + 2), season_text, font=load_font(int(f_title.size * 0.6), bold=True), fill=accent)
    ...
    genre_colors = [GOLD, accent, (255, 255, 100)]
