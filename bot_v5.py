...
# ─── POSTER STYLE 3: INFO CARD (like reference image) ────────────────────────
def build_poster_card(anime: dict, photo_bytes: bytes, brand: str, theme_name: str = "cyan") -> bytes:
    theme  = THEMES.get(theme_name, THEMES["cyan"])
    ACCENT = theme["accent"]
    BG     = theme["bg"]
    ...
    # Season detection
    season_match = re.search(r'\b(season\s*\d+|\d+(?:st|nd|rd|th)\s*season)\b', title, flags=re.IGNORECASE)
    season_text = None
    clean_title = title
    if season_match:
        num = re.search(r'\d+', season_match.group(0)).group(0)
        season_text = f"SEASON {num}"
        clean_title = re.sub(r'\s*\b(season\s*\d+|\d+(?:st|nd|rd|th)\s*season)\b', '', title, flags=re.IGNORECASE).strip()

    # ── Step 1: Blurred background from photo ──
    bg_img = Image.open(io.BytesIO(photo_bytes)).convert("RGB")
    bg_img = bg_img.resize((W, H), Image.LANCZOS)
    bg_img = bg_img.filter(ImageFilter.GaussianBlur(22))
    # Dark overlay
    dark = Image.new("RGB", (W, H), (10, 10, 14))
    base = Image.blend(bg_img, dark, 0.72)

    draw = ImageDraw.Draw(base)

    # ── Step 2: Brand badge top-left ──
    badge_x, badge_y = 60, 42
    badge_text = f"◆  {brand.upper()}"
    f_badge = load_font(15, bold=True)
    btw = draw.textbbox((0, 0), badge_text, font=f_badge)[2]
    bth = 38
    draw.rounded_rectangle([badge_x, badge_y, badge_x + btw + 32, badge_y + bth],
                            radius=19, outline=ACCENT, width=2)
    draw.text((badge_x + 16, badge_y + bth // 2), badge_text, font=f_badge, fill=WHITE, anchor="lm")

    # ── Step 3: Large title ──
    title_upper = clean_title.upper()
    f_title = load_font(88, bold=True)
    max_title_w = int(W * 0.58)
    ...
    ty = badge_y + bth + 28
    draw.text((60, ty), line1, font=f_title, fill=WHITE)
    title_bottom = draw.textbbox((60, ty), line1, font=f_title)[3]
    if line2:
        draw.text((60, title_bottom + 2), line2, font=f_title, fill=WHITE)
        title_bottom = draw.textbbox((60, title_bottom + 2), line2, font=f_title)[3]
    if season_text:
        f_s = load_font(int(f_title.size * 0.45), bold=True)
        draw.text((60, title_bottom + 6), season_text, font=f_s, fill=ACCENT)
        title_bottom = draw.textbbox((60, title_bottom + 6), season_text, font=f_s)[3]

    # ── Step 4: Genre pills ──
    gx = 60
    gy = title_bottom + 22
    f_genre = load_font(13, bold=True)
    for g in genres:
        gtw = draw.textbbox((0, 0), g.upper(), font=f_genre)[2]
        pw = gtw + 26
        ph = 34
        # Pill background (semi-transparent)
        pill_img = Image.new("RGBA", (pw, ph), (*ACCENT, 40))
        pmask = Image.new("L", (pw, ph), 0)
        ImageDraw.Draw(pmask).rounded_rectangle([0, 0, pw, ph], radius=17, fill=255)
        base.paste(Image.new("RGB", (pw, ph), tuple(max(0, c - 180) for c in ACCENT)),
                   (gx, gy), pmask)
        draw = ImageDraw.Draw(base)
        draw.rounded_rectangle([gx, gy, gx + pw, gy + ph], radius=17, outline=ACCENT, width=2)
        draw.text((gx + pw // 2, gy + ph // 2), g.upper(), font=f_genre, fill=WHITE, anchor="mm")
        gx += pw + 12

    # ── Step 5: Synopsis glassmorphism card ──
    card_x = 60
    card_y = gy + ph + 24
    card_w = int(W * 0.56)
    card_h = 150
    ...
    blurred_region = region.filter(ImageFilter.GaussianBlur(8))
    ...
    blurred_region.paste(glass.convert("RGB"), (0, 0), cmask)
    base.paste(blurred_region, (card_x, card_y), cmask)
    draw = ImageDraw.Draw(base)
    draw.rounded_rectangle([card_x, card_y, card_x + card_w, card_y + card_h],
                           radius=16, outline=(255, 255, 255, 60), width=1)

    f_syn = load_font(14)
    wrapped = textwrap.wrap(syn_clean, width=68)
    sy = card_y + 18
    for line in wrapped[:5]:
        draw.text((card_x + 18, sy), line, font=f_syn, fill=(220, 220, 220))
        sy += 22
    if len(wrapped) > 5:
        draw.text((card_x + 18, sy), "...read more", font=f_syn, fill=ACCENT)
    ...
    col_w_meta = card_w // len(meta_items)
    f_mlabel = load_font(11, bold=True)
    f_mval   = load_font(16, bold=True)
    for idx, (label, val) in enumerate(meta_items):
        mx = card_x + idx * col_w_meta
        draw.text((mx, meta_y), label, font=f_mlabel, fill=(130, 130, 130))
        # Score in accent color
        val_color = ACCENT if label == "SCORE" else WHITE
        draw.text((mx, meta_y + 18), val, font=f_mval, fill=val_color)

    # ── Step 7: Right side — vertical portrait poster ──
    poster_x = int(W * 0.63)
    poster_y = 40
    poster_w = W - poster_x - 40
    poster_h = H - 110

    # Load and fit poster image (portrait crop)
    ...
    if img_ratio > target_ratio:
        ...
        p_img = p_img.crop((left, 0, left + new_w, new_h))
    else:
        new_w = p_img.width
        new_h = int(new_w / target_ratio)
        top = (p_img.height - new_h) // 2
        p_img = p_img.crop((0, top, new_w, top + new_h))
    p_img = p_img.resize((poster_w, poster_h), Image.LANCZOS)

    # Rounded mask
    pmask2 = Image.new("L", (poster_w, poster_h), 0)
    ImageDraw.Draw(pmask2).rounded_rectangle([0, 0, poster_w, poster_h], radius=18, fill=255)

    # Shadow
    shadow = Image.new("RGBA", (poster_w + 16, poster_h + 16), (0, 0, 0, 0))
    ImageDraw.Draw(shadow).rounded_rectangle([8, 8, poster_w + 8, poster_h + 8], radius=20, fill=(0, 0, 0, 120))
    shadow = shadow.filter(ImageFilter.GaussianBlur(10))
    base.paste(shadow.convert("RGB"), (poster_x - 8, poster_y - 8),
    ...
    # Decorative corner lines (like in reference)
    cl = 28  # corner line length
    cw = 2
    # Top-right corner
    cx2, cy2 = poster_x + poster_w + 14, poster_y - 6
    draw.line([(cx2, cy2), (cx2 + cl, cy2)], fill=ACCENT, width=cw)
    draw.line([(cx2 + cl, cy2), (cx2 + cl, cy2 + cl)], fill=ACCENT, width=cw)
    # Bottom-right corner
    bx2, by2 = poster_x + poster_w + 14, poster_y + poster_h + 6
    draw.line([(bx2, by2), (bx2 + cl, by2)], fill=ACCENT, width=cw)
    draw.line([(bx2 + cl, by2 - cl), (bx2 + cl, by2)], fill=ACCENT, width=cw)
...
async def received_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("🃏 Style 3 — Info Card", callback_data="style_card")],
    ]
    await update.message.reply_text(
...
async def color_selected(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if style == "ui":
            ...
            poster_bytes = build_poster_card(anime, photo_bytes, brand, theme_name)
        else:
