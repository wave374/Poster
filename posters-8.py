...
# ─── TRACKED TEXT (manual letter-spacing) ─────────────────────────────────────
def draw_tracked(draw, xy, text, font, fill, extra=4):
    x, y = xy
    for ch in text:
        draw.text((x, y), ch, font=font, fill=fill)
        x += draw.textbbox((0, 0), ch, font=font)[2] + extra
    return x
...
def build_poster_banner(anime, photo_bytes, brand, theme_name="cyan"):
    ...
    # Pure black base tint instead of theme color (BG_DARK) — keeps shadow neutral
    base = Image.blend(bg_blur, Image.new("RGB",(W,H),(0,0,0)), 0.72)
    ...
    # Left gradient — pure black fade, no theme-color tint
...
# ═══════════════════════════════════════════════════════════════════════════════
# STYLE 5 — Anime Cruise (glassmorphism split / OTT promo banner)
# Matches the "ANIME CRUISE" / "OTAKU FLIX NETWORK" reference posters exactly:
# full-bleed character art, dark left gradient, huge tight title, JOIN NOW! CTA.
# ═══════════════════════════════════════════════════════════════════════════════
NETWORK_NAME = "OTAKU FLIX NETWORK"   # footer text — edit this to rebrand the footer

def build_poster_cruise(anime, photo_bytes, brand, theme_name="blue"):
    theme  = THEMES.get(theme_name, THEMES["blue"])
    ACCENT = theme["accent"]      # CTA text / play icon color (#3A86FF for "blue")
    DARK   = (16, 16, 24)         # #101018 — fixed dark overlay per spec

    CW, CH = 1280, 640            # 2:1 cinematic banner canvas
    ...
    # ── Background: cover-crop uploaded art to canvas, slight blur + saturation boost
    src = Image.open(io.BytesIO(photo_bytes)).convert("RGB")
    ratio, canvas_ratio = src.width / src.height, CW / CH
    if ratio > canvas_ratio:
        new_h = CH; new_w = int(ratio * CH)
    else:
        new_w = CW; new_h = int(CW / ratio)
    ...
    # ── Dark blue/black gradient overlay — left ~62% width, fading to transparent
    ...
    for x in range(grad_w):
        ...
        ov_draw.line([(x, 0), (x, CH)], fill=(DARK[0], DARK[1], DARK[2], alpha))
    base = Image.alpha_composite(base, overlay)

    # ── Subtle geometric triangle pattern behind text
    tri = Image.new("RGBA", (CW, CH), (0, 0, 0, 0))
    td  = ImageDraw.Draw(tri)
    tsize = 160
    for i in range(-1, 6):
        x0 = i * tsize * 0.75
        td.polygon([(x0, CH), (x0 + tsize, CH), (x0 + tsize / 2, CH - tsize * 1.7)],
                   fill=(255, 255, 255, 12))
    base = Image.alpha_composite(base, tri)

    # ── Slight vignette for cinematic depth
    vig = Image.new("L", (CW, CH), 0)
    vd  = ImageDraw.Draw(vig)
    for i in range(90):
        vd.rectangle([i, i, CW - i, CH - i], outline=int(255 * (i / 90) ** 2))
    vig = vig.filter(ImageFilter.GaussianBlur(30))
    black = Image.new("RGBA", (CW, CH), (0, 0, 0, 150))
    ...
    # ── Logo (top-left) — bold caps + underline
    f_logo = load_font(28, True)
    logo_text = brand.upper()
    draw.text((LEFT, 30), logo_text, font=f_logo, fill=WHITE)
    logo_w = draw.textbbox((0, 0), logo_text, font=f_logo)[2]
    uy = 30 + f_logo.size + 10
    draw.line([(LEFT, uy), (LEFT + logo_w, uy)], fill=WHITE, width=2)

    # ── Season text — small uppercase, letter-spaced
    f_season = load_font(17, True)
    sy = uy + 26
    draw_tracked(draw, (LEFT, sy), season_text, f_season, WHITE, extra=4)

    # ── Anime title — huge, tight tracking, white, soft drop shadow
    title_upper = clean_title.upper()
    max_title_w = int(CW * 0.46)
    f_title = load_font(120, True)
    while f_title.size > 40:
        if draw.textbbox((0, 0), title_upper, font=f_title)[2] <= max_title_w:
            break
        f_title = load_font(f_title.size - 5, True)
    ty = sy + f_season.size + 22

    shadow_layer = Image.new("RGBA", (CW, CH), (0, 0, 0, 0))
    ImageDraw.Draw(shadow_layer).text((LEFT + 4, ty + 4), title_upper, font=f_title, fill=(0, 0, 0, 140))
    ...
    draw = ImageDraw.Draw(base)
    draw.text((LEFT, ty), title_upper, font=f_title, fill=WHITE)
    title_bottom = draw.textbbox((LEFT, ty), title_upper, font=f_title)[3]

    # ── Subtitle
    f_sub = load_font(18)
    subtitle = "Visit Our Channel For More Content Like This..!"
    max_sub_w = int(CW * 0.46)
    while draw.textbbox((0, 0), subtitle, font=f_sub)[2] > max_sub_w and f_sub.size > 12:
        f_sub = load_font(f_sub.size - 1)
    sub_y = title_bottom + 20
    draw.text((LEFT, sub_y), subtitle, font=f_sub, fill=WHITE)

    # ── CTA row: 2x2 grid icon (40% opacity) + JOIN NOW! pill + play circle
    cta_y = sub_y + 70
    grid_layer = Image.new("RGBA", (CW, CH), (0, 0, 0, 0))
    ...
    for row in range(2):
        for col in range(2):
            x0 = gx0 + col * (sq + gap)
            y0 = gy0 + row * (sq + gap)
            gd.rounded_rectangle([x0, y0, x0 + sq, y0 + sq], radius=4, fill=(255, 255, 255, 102))
    base = Image.alpha_composite(base.convert("RGBA"), grid_layer).convert("RGB")
    draw = ImageDraw.Draw(base)
    grid_w = sq * 2 + gap
    grid_h = sq * 2 + gap

    f_btn = load_font(20, True)
    btn_text = "JOIN NOW!"
    btw = draw.textbbox((0, 0), btn_text, font=f_btn)[2]
    pill_w, pill_h = btw + 64, 50
    pill_x = gx0 + grid_w + 26
    pill_y = gy0 + grid_h // 2 - pill_h // 2
    draw.rounded_rectangle([pill_x, pill_y, pill_x + pill_w, pill_y + pill_h],
                            radius=pill_h // 2, fill=WHITE)
    draw.text((pill_x + pill_w // 2, pill_y + pill_h // 2), btn_text,
               font=f_btn, fill=ACCENT, anchor="mm")

    circ_d = 44
    circ_x = pill_x + pill_w + 18
    circ_y = pill_y + pill_h // 2 - circ_d // 2
    draw.ellipse([circ_x, circ_y, circ_x + circ_d, circ_y + circ_d], fill=WHITE)
    cxm, cym = circ_x + circ_d // 2, circ_y + circ_d // 2
    tri_r = 10
    draw.polygon([
        (cxm - tri_r * 0.5, cym - tri_r),
        (cxm - tri_r * 0.5, cym + tri_r),
        (cxm + tri_r, cym),
    ], fill=ACCENT)

    # ── Footer — bottom-left, letter-spaced, semibold
    f_footer = load_font(16, True)
    fy = CH - 44
    draw_tracked(draw, (LEFT, fy), NETWORK_NAME, f_footer, WHITE, extra=6)
