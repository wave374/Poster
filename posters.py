...
def build_poster_banner(anime, photo_bytes, brand, theme_name="cyan"):
    ...
    # Increased from 0.72 -> 0.82 for darker overall base
    base = Image.blend(bg_blur, Image.new("RGB",(W,H),BG_DARK), 0.82)
    ...
    # Left gradient — wider (0.6->0.72), stronger alpha (200->255), steeper falloff (0.7->0.45)
    ov2 = Image.new("RGBA",(W,H),(0,0,0,0))
    for x in range(int(W*0.72)):
        a = int(255*(1-x/(W*0.72))**0.45)
    ...
    # Extra pure-black layer far left for max text readability
    black_ov = Image.new("RGBA",(W,H),(0,0,0,0))
    for x in range(int(W*0.40)):
        a = int(130*(1-x/(W*0.40))**0.6)
        ImageDraw.Draw(black_ov).line([(x,0),(x,H)],fill=(0,0,0,a))
    base = Image.alpha_composite(base.convert("RGBA"),black_ov).convert("RGB")
    draw = ImageDraw.Draw(base)
